...
function Darius:LoadMenu()
	--Main Menu
	self.Menu = MenuElement({type = MENU, id = "Menu", name = "Darius"})
	--Main Menu-- Darius
	self.Menu:MenuElement({type = MENU, id = "Mode", name = "Kharneth's Darius"})
	--Main Menu-- Darius -- Combo
	...
	--Main Menu-- Darius -- LaneClear
        self.Menu.Mode:MenuElement({type = MENU, id = "LaneClear", name = "Lane Clear"})
	self.Menu.Mode.LaneClear:MenuElement({id = "Q", name = "Use Q", value = true})self.Menu.Mode:MenuElement({type = MENU, id = "LaneClear", name = "Lane Clear"})
...
function Darius:GetValidMinion(range)
...
function Darius:CountEnemyMinions(range)
...
function Darius:Combo()
	...
	local target =  (_G.SDK and _G.SDK.TargetSelector:GetTarget(1200, _G.SDK.DAMAGE_TYPE_PHYSICAL)) or (_G.GOS and _G.GOS:GetTarget(1200,"AD")) or ( _G.EOWLoaded and EOW:GetTarget())
	    ...
	    if self:IsValidTarget(target,W.range*0.9) and self.Menu.Mode.Combo.W:Value() and self:isReady(_W) and myHero.attackData.state == STATE_WINDDOWN  then
...
function Darius:HpPred(unit, delay)
	if _G.GOS then
...
function Darius:Draw()
	if myHero.dead then return end
		if self.Menu.Drawing.E:Value() then Draw.Circle(myHero.pos, E.range, self.Menu.Drawing.Width:Value(), self.Menu.Drawing.Color:Value())	
		end	
end
